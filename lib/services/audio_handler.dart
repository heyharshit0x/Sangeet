...
Future<AudioHandler> initAudioService() async {
  return await AudioService.init(
    ...
    config: AudioServiceConfig(
      ...
      androidNotificationChannelId: 'com.sangeet.app.audio',
      androidNotificationChannelName: 'Sangeet Notification',
...
class MyAudioHandler extends BaseAudioHandler with GetxServiceMixin {
  ...
  void _notifyAudioHandlerAboutPlaybackEvents() {
    _player.playbackEventStream.listen((PlaybackEvent event) {
      ...
      if (e is PlayerException) {
        ...
        if (isNetworkError) {
          printINFO("Network error detected, attempting to recover...");
          ...
          for (int attempt = 0; attempt < 3; attempt++) {
            ...
            try {
              ...
              await _player.seek(curPos, index: 0);
              printINFO("Successfully recovered from network error");
              return;
            } catch (retryError) {
              printERROR("Retry attempt ${attempt + 1} failed: $retryError");
            }
          }
          ...
          // For non-network errors, try to get a new URL
  ...
  void _listenToPlaybackForNextSong() {
    final playerDurationOffset = GetPlatform.isWindows ? 200 : 0;
