...
class HomeScreenController extends GetxController {
  ...
  // Filter functionality for Spotify-style home
  final selectedFilter = "All".obs;

  // Recent songs functionality
  final recentSongs = <MediaItem>[].obs;
  ...
  List<dynamic> getFilteredContent() {
    if (selectedFilter.value == "All") {
      return [...middleContent, ...fixedContent];
    } else if (selectedFilter.value == "Music") {
      // Filter to show only albums
      return [...middleContent, ...fixedContent]
          .where((item) => item.runtimeType.toString() == "AlbumContent")
          .toList();
    } else if (selectedFilter.value == "Artists") {
      // Show artist-related content (could be enhanced later)
      return [...middleContent, ...fixedContent];
    } else {
      // Podcasts or other filters
      return [...middleContent, ...fixedContent];
    }
  }

  Album? getFeaturedAlbum() {
    for (var content in middleContent) {
      if (content.runtimeType.toString() == "AlbumContent") {
        final albumContent = content as AlbumContent;
        if (albumContent.albumList.isNotEmpty) {
          return albumContent.albumList.first;
        }
      }
    }
    return null;
  ...
  Future<void> loadRecentSongs() async {
    final box = Hive.box("AppPrefs");
    final recentSongsData = box.get("recentSongs", defaultValue: []);

    if (recentSongsData is List && recentSongsData.isNotEmpty) {
      // Filter out songs older than 1 week
      final oneWeekAgo = DateTime.now().subtract(const Duration(days: 7));
      final validRecents = recentSongsData.where((item) {
        final timestamp = item['timestamp'] as int?;
        if (timestamp == null) return false;
        final songDate = DateTime.fromMillisecondsSinceEpoch(timestamp);
        return songDate.isAfter(oneWeekAgo);
      }).toList();

      recentSongs.value = validRecents
          .map((e) => MediaItemBuilder.fromJson(e['song']))
          .toList()
          .cast<MediaItem>();

      // Save cleaned list back
      if (validRecents.length != recentSongsData.length) {
        await box.put("recentSongs", validRecents);
      }
    }
  }

  Future<void> addRecentSong(MediaItem song) async {
    ...
    if (recentSongs.length > 50) {
      recentSongs.value = recentSongs.take(50).toList();
    }

    // Save to storage
    final box = Hive.box("AppPrefs");
    final recentSongsData = recentSongs
        .map((s) => {
              'song': MediaItemBuilder.toJson(s),
              'timestamp': DateTime.now().millisecondsSinceEpoch,
            })
        .toList();
    await box.put("recentSongs", recentSongsData);
  }

  void deleteRecentSong(MediaItem song) async {
    recentSongs.removeWhere((s) => s.id == song.id);
